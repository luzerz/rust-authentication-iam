...
mod tests {
    ...
    async fn test_in_memory_role_repository_set_parent_role() {
        ...
        // Set parent role
        let result = repo
            .set_parent_role(&child_role.id, Some(&parent_role.id))
            .await;
        assert!(result.is_ok());
    ...
    async fn test_in_memory_role_repository_get_inherited_roles() {
        ...
        repo.set_parent_role(&parent_role.id, Some(&grandparent_role.id))
            .await
            .unwrap();
        repo.set_parent_role(&child_role.id, Some(&parent_role.id))
            .await
            .unwrap();
    ...
    async fn test_in_memory_role_repository_would_create_cycle() {
        ...
        let would_create_cycle = repo
            .would_create_cycle(&parent_role.id, &child_role.id)
            .await
            .unwrap();
    ...
    async fn test_in_memory_role_repository_complex_hierarchy() {
        ...
        repo.set_parent_role(&manager.id, Some(&admin.id))
            .await
            .unwrap();
        repo.set_parent_role(&user.id, Some(&manager.id))
            .await
            .unwrap();
        repo.set_parent_role(&guest.id, Some(&user.id))
            .await
            .unwrap();
